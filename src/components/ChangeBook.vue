<template>
  <div class="text-normal font-semibold text-center mt-4 mb-7">创建学习集1</div>
  <div class="mx-6">
...
import { infor, create } from "@/api/api";
...
const getInfo = async () => {
  try {
    ...
    state.booksInfo = resp.data;
    state.book.name = resp.data.name;
    state.book.recommend = resp.data.recommend;
    state.book.describe = resp.data.describe;
    state.book.price = resp.data.price;
    state.book.cover = resp.data.cover;
    state.book.tags = resp.data.tags;
  } catch (err) {
    console.log(err);
  }
};
